...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x_q: Tensor) -> Tensor:
        ...
        key=self.depth_conv(key)
        value=self.depth_conv(value)
        ...
        #key = self.key(x_q).reshape(b_sz, n_patches, self.num_heads, self.qk_dim // self.num_heads).permute(0, 2, 1, 3)

        #value = self.value(x_q).reshape(b_sz, n_patches, self.num_heads, in_channels // self.num_heads).permute(0, 2, 1, 3)
